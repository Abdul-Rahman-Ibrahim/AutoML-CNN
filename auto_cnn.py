...
class CNN(nn.Module):
    ...
    def __init__(self, 
        ...
        for i in range(len(conv_sizes)):
            ...
            self.conv_layers.append(nn.Conv2d(in_channel, out_channel, kernel_size=kernel_size, padding=padding, stride=stride))
            self.pool_layers.append(nn.MaxPool2d(kernel_size=pool_sizes[i]))
            in_channel = out_channel

            conv_output_size = CNN.output_size_after_conv(current_shape, kernel_size, stride, padding)
            if pool_sizes[i] == 0:
                current_shape = conv_output_size
                continue
            pool_output_size = CNN.output_size_after_pool(conv_output_size, pool_sizes[i])
    ...
    def train(self, train_data, epochs, batch_size, eta, test_data=None):
        if test_data:
            self.train_with_test(train_data, epochs, batch_size, eta, test_data)
        else:
